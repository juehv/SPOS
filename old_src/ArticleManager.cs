...
namespace SimplePOS
{
  ...
  public class ArticleManager : Window, IComponentConnector
  {
    ...
    {
      this.items = DAO_.getInstance().getAllArticles();
    ...
    private void button2_Click(object sender, RoutedEventArgs e)
    {
      ...
      if (selectedIndex == -1)
        return;
      AddArticle addArticle = new AddArticle(this.items[selectedIndex]);
      addArticle.Owner = (Window) this;
      addArticle.ShowDialog();
      this.updateGrid();
    }
    ...
    {
      int selectedIndex = this.dataGrid1.SelectedIndex;
      if (selectedIndex == -1 || MessageBox.Show("Möchten Sie den Artikel wirklich entfernen?", "Artikel entfernen?", MessageBoxButton.YesNo) != MessageBoxResult.Yes)
        return;
      DAO_.getInstance().deleteArticleByNumber(this.items[selectedIndex].Number);
      this.updateGrid();
    }
    ...
    public void InitializeComponent()
    {
      ...
      Application.LoadComponent((object) this, new Uri("/SimplePOS;component/article/articlemanager.xaml", UriKind.Relative));
    }
