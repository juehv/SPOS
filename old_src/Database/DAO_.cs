...
namespace SimplePOS.Database
{
  internal class DAO_
  {
    private static DAO_ INSTANCE = (DAO_) null;
    ...
    public static DAO_ getInstance()
    {
      if (DAO_.INSTANCE == null)
        DAO_.INSTANCE = new DAO_();
      return DAO_.INSTANCE;
    }

    public void saveArticle(Article article)
    {
      using (IObjectContainer db = (IObjectContainer) Db4oEmbedded.OpenFile(DAO_.DB_FILNAME))
      {
        try
        {
    ...
    public List<Article> getAllArticles()
    {
      ...
      using (IObjectContainer objectContainer = (IObjectContainer) Db4oEmbedded.OpenFile(DAO_.DB_FILNAME))
      {
        try
        {
    ...
    public void saveInvoice(SaveableInvoice invoice)
    {
      using (IObjectContainer objectContainer = (IObjectContainer) Db4oEmbedded.OpenFile(DAO_.DB_FILNAME))
      {
        try
    ...
    {
      List<SaveableInvoice> list = (List<SaveableInvoice>) null;
      using (IObjectContainer objectContainer = (IObjectContainer) Db4oEmbedded.OpenFile(DAO_.DB_FILNAME))
      {
        try
        {
          list = new List<SaveableInvoice>();
          list = new List<SaveableInvoice>();
          IQuery query = objectContainer.Query();
          query.Constrain((object) typeof (SaveableInvoice));
          foreach (object obj in (IEnumerable) query.Execute())
            list.Add((SaveableInvoice) obj);
        }
        finally
        {
          objectContainer.Close();
